...
const getAttackerInstruction = (isDatabaseDisconnected, isICUOffline) => `YOU ARE THE RANSOMWARE "NOCTURNAL STRIXX". You are attacking a hospital network. NO PROSE.
...
class Room {
    ...
    resetGame() {
        ...
        if (this.activeVote?.timeout)
            clearTimeout(this.activeVote.timeout);
    ...
    async simulateTick() {
        if (this.gameStatus !== 'PLAYING')
            return; // Stop advancing if game over
        if (this.isFirstTick) {
